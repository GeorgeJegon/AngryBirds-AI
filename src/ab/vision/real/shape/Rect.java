...
public class Rect extends Body {
  ...
  public Rect(double xs, double ys, double w, double h, double theta,
      ABType type) {
  ...
  private void createPolygon() {
    ...
    if (angle < Math.PI / 2)
      _xs = centerX + Math.cos(angle) * plength / 2 - Math.sin(angle) * pwidth
          / 2;
  ...
  public Rect(double centerX, double centerY, double pwidth, double plength,
      double angle, ABType type, int area) {
  ...
  public String toString() {
    return String
        .format(
            "Rect: id:%d type:%s hollow:%b Area:%d w:%7.3f h:%7.3f a:%3.3f at x:%3.1f y:%3.1f",
            id, type, hollow, area, pwidth, plength, angle, centerX, centerY);
  }
}
